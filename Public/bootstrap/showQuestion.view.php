		...
		<script type="text/javascript">
			function showData(page){
				...
				for(var i=0;i<page.data.length;i++){
					$("#tableData").append(
						"<tr>"+
     			    	"<td>"+ page.data[i].qid +"</td>"+
     			        "<td>"+ page.data[i].content +"</td>"+
      			        "<td>"+ (page.data[i].type==1?"单选":"多选") +"</td>"+
     			    	"<td>"+ page.data[i].answer +"</td>"+
     			    	"<td>"+ page.data[i].realName +"</td>"+
     			    	"<td>"+ page.data[i].courseName +"</td>"+
     			    	"<td>"+ page.data[i].questionTime +"</td>"+
     			    	"<td>"+ page.data[i].parse +"</td>"+
     			    	"<td><a class='btn btn-info btn-xs' href='updateQuestionForm.php?qid="+page.data[i].qid+"'>更新</a>&nbsp;&nbsp;<a class='btn btn-danger btn-xs' href='javascript:delEmpById("+page.data[i].qid+");'>删除</a></td>"+
     			    	"</tr>");
				}
				//分页导航条的处理
			...
			$(function(){
				//页面加载时默认展示第一页 无搜索条件
				$.post("../index.php",{c:"question",a:"showQuestion",pageNo:1,pageSize:10},function(page){
					showData(page);
				},"json");
			...
			<caption>
				<form id="searchForm">
    				...
    				<div class="form-group col-md-7">
    					...
    					<a href="insertQuestionForm.php" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-plus"></span>&nbsp;新增</a>
    				</div>
				</form>
			</caption>
			<thead>
